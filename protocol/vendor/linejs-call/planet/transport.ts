...
import type { CallTransport } from "../call_transport.ts";
...
/** SSRCs advertised in packNativeSetupOffer for 1:1 calls.
 * Outbound RTP must use these — not the peer's rtpPort/rtcpId — or the
 * responder keeps retransmitting CONN_REQ and releases with 511
 * (CONN_REQ_FAIL.timeout / T103) after ~10s.
 */
const LOCAL_1TO1_AUDIO_RTP_SSRC = 101;
const LOCAL_1TO1_AUDIO_RTCP_SSRC = 201;
const LOCAL_1TO1_VIDEO_RTP_SSRC = 111;
const LOCAL_1TO1_VIDEO_RTCP_SSRC = 211;
const LOCAL_1TO1_DATA_RTP_SSRC = 121;
const LOCAL_1TO1_DATA_RTCP_SSRC = 221;
...
function defaultOneToOneStrmSpec(peerOffer?: NativeSetupOffer): Uint8Array {
	const state = { paused: false, code: 0 };
	const peerAudio =
		peerOffer?.media.find((m) => m.name === "A" && m.enabled !== 0) ??
		peerOffer?.media.find((m) => m.kind === 1 && m.enabled !== 0);
	const peerVideo =
		peerOffer?.media.find((m) => m.name === "V") ??
		peerOffer?.media.find((m) => m.kind === 2);
	const peerData =
		peerOffer?.media.find((m) => m.name === "D" && m.enabled !== 0) ??
		peerOffer?.media.find((m) => m.kind === 3 || m.kind === 6);
	// strms = streams we receive (peer RTP SSRCs).
	// txStrms = our RTCP SSRCs (match local SETUP offer).
	return packStrmSpec({
		strms: [
			{
				ssrc: peerAudio?.rtpPort ?? 102,
				bitrate: { target: 32 },
				state,
				ptime: 40,
		...
		txStrms: [
			{
				ssrc: LOCAL_1TO1_AUDIO_RTCP_SSRC,
				state,
				retx: { reqdOn: true, reqdRttThre: 300 },
			},
			{
				ssrc: LOCAL_1TO1_VIDEO_RTCP_SSRC,
				state,
				retx: { reqdOn: false },
			},
		],
...
function defaultOneToOneDataSessionPayload(
	peerOffer?: NativeSetupOffer,
): Uint8Array {
	return packMcDataSessionPayload(defaultOneToOneStrmSpec(peerOffer));
}
...
export class PlanetTransport implements CallTransport {
	...
	/** Peer's NativeSetupOffer from CONN_REQ — used for MC strmSpec SSRCs. */
	#peerMediaOffer?: NativeSetupOffer;
	#localMediaChanId = 1n;
	...
	#dataSrtpRecvCandidates: SrtpCryptoContext[] = [];
	#dataRtpPayloadType = 98;
	#mediaKeyMode?: PlanetMediaKeyMode;
	...
	#targetMid?: string;
	#closed = true;
	/** Hangup requested — if SETUP lands after close started, still send REL. */
	#cancelAfterSetup = false;
	...
	async #decryptMediaRtp(
		...
		try {
			...
			// line-gtk patch: upstream derives DATA SRTP but never tries it.
			const dataContexts = [
				this.#dataSrtpRecv,
				...this.#dataSrtpRecvCandidates,
			].filter((c): c is SrtpCryptoContext => !!c);
			for (const ctx of dataContexts) {
				try {
					const rtp = await srtpDecrypt(ctx, wire);
					return { rtp, mode: "data", switched: false };
				} catch {
					/* try next DATA context */
				}
			}
			if (this.#mediaKeyMode !== "auto") throw audioErr;
		}
	...
	async #sendSetup(opts: { to: string }): Promise<void> {
		...
		// Mark before send so a concurrent hangup/close still emits REL_REQ.
		this.#setupSent = true;
		await this.#sendEnvelope(
			{ kind: "cc", data: ccMsg },
			{ bootstrap: true, msgId: CASSINI_MSG_ID_SETUP_REQ },
		);
		if (this.#cancelAfterSetup) {
			await this.#sendRelReq().catch(() => {});
		}
	}

	async #sendRelReq(): Promise<void> {
		if (!this.#route || !(this.#sock || this.#opts.wireSend)) return;
		...
		this.#debug({
			type: "rel_req_sent",
			releaser: this.#groupJoined ? "participant" : "initiator",
			relCode: this.#groupJoined ? 1 : 2,
		});
		await this.#sendEnvelope(
			{ kind: "cc", data: ccMsg },
	...
	async waitForAnswerDetailed(opts: {
		...
		if (opts.autoConnRsp ?? true) {
			...
			// Native opens the BEPI/MC channel soon after CONN_RSP; don't wait
			// solely for a peer JOIN_REQ that never arrives if CONN looked wrong.
	...
	async #configureMedia(
		...
		if (
			...
			for (const selection of Object.values(MEDIA_KEY_SELECTIONS)) {
				...
				this.#dataSrtpRecvCandidates.push(
					await deriveSrtpContext(
						derivePlanetMediaStreamKeying(recvKeying, "DATA"),
					),
					await deriveSrtpContext(
						derivePlanetMediaStreamKeying(sendKeying, "DATA"),
					),
				);
			}
		}
		...
		// line-gtk: 1:1 also needs DATA send+recv (upstream only set recv via mediaSecret).
		if (!this.#groupJoined) {
			if (
				local.material.mediaSecret.length === 30 &&
				peerOffer.mediaSecret?.length === 30
				...
				// Fall back to ECDH DATA stream keying matching audio-reverse-stage.
				const keyInput = {
					local: {
						privateKey: local.keypair.privateKey,
						publicKey: local.material.mediaPubKey,
				...
				const variants = derivePlanetMediaKeyingVariants(keyInput);
				const sel = MEDIA_KEY_SELECTIONS["reverse-stage"];
				const sendKeying = variants.variants[sel.send];
				const recvKeying = variants.variants[sel.recv];
				this.#groupDataSrtpSend = await deriveSrtpContext(
					derivePlanetMediaStreamKeying(sendKeying, "DATA"),
				);
				this.#dataSrtpRecv = await deriveSrtpContext(
		...
		const data =
			peerOffer.media.find((m) =>
				...
				m.kind === 3 && m.enabled !== 0 && m.rtpId !== undefined
			);
		if (data?.rtpId !== undefined) {
			this.#dataRtpPayloadType = data.rtpId;
		}
		if (this.#groupJoined) {
		...
		const ccMsg = packPlanetCcMsg(
			{
				...
				// Native app keeps conn_rsp header srcChanId == body mChanId.
	...
	async #sendMcDataRsp(
		...
		const data = defaultOneToOneDataSessionPayload(this.#peerMediaOffer);
	...
	async #sendMcJoinRsp(
		...
		const rsp = packMcJoinRsp({
			result: 0,
			data: defaultOneToOneStrmSpec(this.#peerMediaOffer),
		});
	...
	async #sendMcCheckRpt(
		...
		const rpt = packMcCheckRpt(defaultOneToOneStrmSpec(this.#peerMediaOffer));
	...
	async close(): Promise<void> {
		this.#closed = true;
		this.#cancelAfterSetup = true;
		this.#clearKeepalive();
		// Unblock invite / waitForAnswer so hangup does not leave SETUP ringing.
		const pending = this.#pending.splice(0);
		for (const w of pending) {
			try {
				w(new Error("PLANET transport closed"));
			} catch { /* ignore */ }
		}
		this.#queued = [];
		try {
			if (this.#setupSent) {
				await this.#sendRelReq();
			}
		} catch (e) {
			this.#debug({
				type: "rel_req_failed",
	...
	async #sendGroupDataRtpControl(): Promise<void> {
		...
		// Group waits for audio RTP extensions; 1:1 has no extensions — send immediately.
		if (this.#groupJoined && this.#groupAudioExtensionIndex < 6) return;
		...
		const payloadType = this.#groupJoined ? 101 : this.#dataRtpPayloadType;
		const rtp = buildRtp({
			payloadType,
			marker: true,
		...
		this.#debug({
			type: this.#groupJoined ? "group_data_rtp_send" : "data_rtp_send",
			bytes: wire.length,
	...
	async *receive(): AsyncIterable<Uint8Array> {
		...
		while (true) {
			...
			try {
				...
				// Never feed DATA/video payloads into the Opus audio path.
				const audioPt = this.#rtp?.payloadType ?? 96;
				if (parsed.payloadType !== audioPt) {
					continue;
				}
